...
public class OAuth2ResourceApplicationTests {
	...
	public void flightsSecuredByDefault() throws Exception {
		this.mvc.perform(get("/secured/1").accept(MediaTypes.HAL_JSON)).andExpect(status().isUnauthorized());
		this.mvc.perform(get("/secured/2").accept(MediaTypes.HAL_JSON)).andExpect(status().isUnauthorized());
