...
import com.chensoul.lss.web.dto.ProjectDto;
...
public class ResourceServerLiveTest {
	...
	private static final String RESOURCE_SERVER_BASE_URL = "http://localhost:8081/resource";
	...
	private static final String AUTHORIZE_URL_PATTERN =
		AUTH_SERVER_BASE_URL + "/oauth/authorize?response_type=code&client_id=client&scope=%s&state=1234&redirect_uri=" + REDIRECT_URL;

	private static final String TOKEN_URL = AUTH_SERVER_BASE_URL + "/oauth/token";
	private static final String RESOURCE_URL = RESOURCE_SERVER_BASE_URL + "/api/projects";
	...
	public void givenUserWithOtherScope_whenGetProjectResource_thenSeeOther() {
		...
		String url2 = URLDecoder.decode(response.getHeader(HttpHeaders.LOCATION));
...
//			.formParams("username", USERNAME, "password", PASSWORD, "_csrf", csrf)
		...
		// obtain authentication code and state
		String authorizeUrl = URLDecoder.decode(response.getHeader(HttpHeaders.LOCATION));
